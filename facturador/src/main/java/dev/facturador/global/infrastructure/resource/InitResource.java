...
public class InitResource {
    ...
    public String verifyTokenIsTrue(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            if (StringUtils.hasText(token)) return jwt.createUserByToken(token);
        ...
        throw new ResponseNull("Token is missing");
    }
     ...
     */
    private LinkedHashMap<String, String> createResponseWithEmail(String email) {
        //Busca al usuario con el email
        var user = ((CustomUserDetails) service.loadUserByUsername(email));

        return (LinkedHashMap<String, String>) Map.of(
