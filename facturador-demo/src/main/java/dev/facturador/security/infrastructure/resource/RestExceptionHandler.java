...
import dev.facturador.security.domain.Errors;
import dev.facturador.security.domain.SaveNotFoundException;
...
public class RestExceptionHandler implements WebExceptionHandler {
    ...
    public Mono<Void> handle(ServerWebExchange exchange, Throwable exception) {
        if (exception instanceof WebExchangeBindException) {
            var webExchangeBindException = (WebExchangeBindException) exception;

            log.debug("errors:" + webExchangeBindException.getFieldErrors());
            var errors = new Errors("validation_failure", "Validation failed.");
            webExchangeBindException.getFieldErrors().forEach(
                    e -> errors.add(e.getField(), e.getCode(), e.getDefaultMessage()));
        ...
        } else if (exception instanceof SaveNotFoundException) {
