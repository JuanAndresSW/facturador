 ...
 */
export default function fetch(method:("GET"|"POST"|"PUT"|"DELETE"), url: string, 
content: {body?:string, token?: string}, callback: Function): void {
  ...
  function handleResponse() {
    if (xhr.readyState === XMLHttpRequest.DONE) {
      ...
      callback(xhr.status, xhr.responseText); console.log(xhr.status + xhr.responseText); return;
    }
  }
}