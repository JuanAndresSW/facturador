...
export default function SignUp() {
    ...
    /*FORMULARIO*****************************************************/
    return (active === "user" ?
        React.createElement(Form, { title: "Datos de la cuenta", onSubmit: validateUser }, React.createElement(Link, { to: "/" }, React.createElement(BiHome, null)), React.createElement(Field, { icon: React.createElement(BiText, null), label: "\u00BFC\u00F3mo quieres que te identifiquemos?", bind: [username, setUsername], validator: Valid.names(username) }), React.createElement(Field, { icon: React.createElement(BiAt, null), label: "Tu direcci\u00F3n de correo electr\u00F3nico", bind: [email, setEmail], validator: Valid.email(email) }), React.createElement(Field, { icon: React.createElement(BiKey, null), label: "Elige una contrase\u00F1a", bind: [password, setPassword], type: "password", validator: Valid.password(password) }), React.createElement(Field, { label: "Vuelve a escribir la contrase\u00F1a", bind: [passwordMatch, setPasswordMatch], type: "password", validator: password === passwordMatch }), React.createElement(Image, { label: "Foto de perfil", note: "(opcional)", setter: setAvatar, img: avatar }), React.createElement(ErrorMessage, { message: userError }), React.createElement(Button, { type: "submit", text: "Siguiente" }), React.createElement("p", { style: { textAlign: 'center', cursor: 'default' } }, '¿Ya tienes una cuenta? ', React.createElement("a", { href: "/ingresar", style: { textDecoration: 'none' } }, "Ingresar")))
        :
            active === "trader" ?
                React.createElement(Form, { title: "Datos del comercio", onSubmit: validateTrader }, sending ? null : React.createElement(BiChevronLeft, { onClick: function () { return setActive("user"); } }), React.createElement(Field, { icon: React.createElement(BiIdCard, null), label: "Escribe tu raz\u00F3n social", bind: [businessName, setBusinessName], validator: Valid.names(businessName) }), React.createElement(Radio, { legend: "Selecciona una categor\u00EDa:", bind: [vatCategory, setVatCategory], options: ["Responsable Inscripto", "Monotributista", "Sujeto Exento"] }), React.createElement(Field, { label: "C.U.I." + (vatCategory === "Monotributista" ? "L." : "T."), note: "(si no eliges uno, se generar\u00E1 uno falso)", bind: [code, setCode], validator: Valid.code(code), icon: React.createElement(BiHash, null) }), React.createElement(Field, { label: "N\u00FAmero de ingresos brutos", note: "(si no eliges uno, se generar\u00E1 uno falso)", bind: [grossIncome, setGrossIncome], icon: React.createElement(BiWallet, null), validator: Valid.code(grossIncome) }), React.createElement(ErrorMessage, { message: traderError }), sending ? React.createElement(Loading, null) : React.createElement(Button, { type: "submit", text: "Enviar" }))
                : null);
}
