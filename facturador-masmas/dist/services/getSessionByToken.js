 ...
 */
export default function getSessionByToken(callback) {
    var access = getToken("access");
    if (!/^[A-Za-z0-9-_]*\.[A-Za-z0-9-_]*\.[A-Za-z0-9-_]*$/.test(access)) {
        callback(false, "El token no contiene un formato válido.");
    }
    else
        ajax("GET", "auth/init", { token: access }, handleAccessTokenResponse);
    function handleAccessTokenResponse(status, data) {
        if (status === 200) {
            success(data);
        }
        else
            ajax("POST", "auth/refresh", { token: getToken("refresh") }, handleRefreshTokenResponse);
    }
    ;
